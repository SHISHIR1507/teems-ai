...
def require_tenant():
    ...
    For long-running operations (e.g., image generation), the token is NOT re-validated during
    execution. This allows operations to complete even if the token expires mid-execution, as
    long as it was valid when the request started.
