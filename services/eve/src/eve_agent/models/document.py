...
class Document(Base):
    """Document model - maps to 'eve_documents' table."""
    
    __tablename__ = "eve_documents"
    ...
    # Relationship - using lazy="selectin" for async compatibility
    chunks = relationship(
        "DocumentChunk",
        back_populates="document",
        cascade="all, delete-orphan",
        lazy="selectin"  # Required for async SQLAlchemy to prevent greenlet errors
    )
...
class DocumentChunk(Base):
    """DocumentChunk model - maps to 'eve_document_chunks' table."""
    
    __tablename__ = "eve_document_chunks"
    
    # Existing columns
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    document_id = Column(UUID(as_uuid=True), ForeignKey("eve_documents.id", ondelete="CASCADE"), nullable=False)
    tenant_id = Column(String(128), nullable=False, index=True)
