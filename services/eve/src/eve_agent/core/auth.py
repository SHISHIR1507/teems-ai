...
class AuthenticatedUser(BaseModel):
    """Subset of the Auth service's AuthenticatedUser, enough for Eve."""
...
class Auth0Client:
    """Minimal Auth0 helper for verifying tokens, modelled after eve-core."""

    def __init__(self, settings: Settings, jwks_ttl: int = 600) -> None:
        self.settings = settings
        self.jwks_ttl = jwks_ttl
        self._jwks_cache: JWKSCache | None = None
        self._jwks_lock = None  # lazily created asyncio.Lock to avoid import cycles
...
def require_tenant():
    ...
    For long-running operations (e.g., video generation, document processing), the token is NOT
